...
describe('ShopFactory and Shop System', () => {
    ...
    it("should create unique item properly", async () => {
        ...
        let expectedFUIContent = beginCell()
            .storeStringTail(firstUniqueItemContent)
            .endCell();
        ...
        console.log("fuItemContent: ", fuItemContent);
        console.log("expectedFUIContent type as string: ", expectedFUIContent);
        expect(fuItemPrice).toEqual(firstUniqueItemPrice);
        expect(fuItemIndex).toEqual(0n);
        expect(fuItemContent.equals(expectedFUIContent));
    });
...
it("should handle multiple shops with multiple orders", async () => {
    ...
    for (let i = 0; i < 3; i++) {
        ...
        const itemAddress = contractAddress(0, await firstShop.getUniqueItemInit(BigInt(i)));
    ...
    for (let i = 0; i < 2; i++) {
        ...
        const itemAddress = contractAddress(0, await secondShop.getUniqueItemInit(BigInt(i)));
...
it("should handle mixed item types in multiple shops", async () => {
    ...
    const techUniqueItemAddress = contractAddress(0, await techShop.getUniqueItemInit(0n));
...
it("should handle multiple shops with complex orders", async () => {
    ...
    for (let i = 0; i < 2; i++) {
        ...
        const itemAddress = contractAddress(0, await shop1.getUniqueItemInit(BigInt(i)));
    ...
    const shop3UniqueItem = contractAddress(0, await shop3.getUniqueItemInit(0n));
...
it("should handle independent shops with separate orders", async () => {
    ...
    const laptopAddress = contractAddress(0, await electronicsShop.getUniqueItemInit(0n));
    const headphonesAddress = contractAddress(0, await electronicsShop.getProductlineItemInit(0n));
    const jacketAddress = contractAddress(0, await clothingShop.getUniqueItemInit(0n));
...
it("should handle multiple complex orders across different shops", async () => {
    ...
    for (let i = 0; i < 4; i++) {
        ...
        const itemAddress = contractAddress(0, await electronicsShop.getUniqueItemInit(BigInt(i)));
    ...
    for (let i = 0; i < 3; i++) {
        ...
        const itemAddress = contractAddress(0, await clothingShop.getUniqueItemInit(BigInt(i)));
    ...
    for (let i = 0; i < 2; i++) {
        ...
        const itemAddress = contractAddress(0, await booksShop.getUniqueItemInit(BigInt(i)));
    ...
    for (let i = 0; i < 5; i++) {
        ...
        const itemAddress = contractAddress(0, await homeShop.getUniqueItemInit(BigInt(i)));
...
it("should handle payment for multiple orders from different shops", async () => {
    ...
    const itemAddress1 = contractAddress(0, await electronicsShop.getUniqueItemInit(0n));
...
it("should handle payment for multiple orders for multiple items from different shops", async () => {
    ...
    for (let i = 0; i < 10; ++i) {
        ...
        const itemAddress1 = contractAddress(0, await electronicsShop.getUniqueItemInit(BigInt(i)));
...
it("should handle complex payment with mixed items from multiple shops", async () => {
    ...
    const fashionItemAddress = contractAddress(0, await fashionShop.getUniqueItemInit(0n));
    ...
    const electronicsItemAddress = contractAddress(0, await electronicsShop.getUniqueItemInit(0n));
...
it("should handle load with multiple orders and payments", async () => {
    ...
    const itemAddress = contractAddress(0, await shop1.getUniqueItemInit(0n));
...
it("should handle moderate order with single item", async () => {
    ...
    const itemAddress = contractAddress(0, await firstShop.getUniqueItemInit(0n));
    uniqueItems.push({ address: itemAddress, price });
